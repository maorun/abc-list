    ...
    return (
      <View>
        ...
        <Overlay isVisible={this.state.createItem}
          ...
          <View>
            <Input onChangeText={(text => this.newText = text)} autoFocus={true} style={{marginBottom: 5}}/>
            <Button title={'Speichern'} onPress={() => this.saveWords(this.newText).then(() => this.toggle())} />
          </View>
        </Overlay>
        <View style={{flex: 1, flexDirection: 'row', flexWrap: 'wrap', marginTop: 5, justifyContent: 'space-around'}}>
          {
            this.state.words.map((text: string) => {
              return (
                <SavedWord key={text} text={text} onDelete={() => {
  ...
  private saveWords(word?: string) {
    return new Promise(((resolve) => {
      ...
      this.setState(this.state, () => {
        AsyncStorage.setItem(this.getCacheKey(), JSON.stringify(this.state.words)).then(() => {
          resolve();
        });
      });
    }));
