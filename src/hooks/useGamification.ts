...
export function useGamification() {
  ...
  const trackActivity = useCallback((
    ...
    try {
      ...
      console.log(`[Gamification] Activity tracked: ${activityType}`, metadata);
    } catch (error) {
  ...
  const trackListCreated = useCallback((listName?: string) => {
    trackActivity('list_created', {listName});
  }, [trackActivity]);

  const trackWordAdded = useCallback((word?: string, listName?: string) => {
    trackActivity('word_added', {word, listName});
  }, [trackActivity]);

  const trackKawaCreated = useCallback((kawaKey?: string) => {
    trackActivity('kawa_created', {kawaKey});
  }, [trackActivity]);

  const trackKagaCreated = useCallback((kagaKey?: string) => {
    trackActivity('kaga_created', {kagaKey});
  }, [trackActivity]);

  const trackStadtLandFlussGame = useCallback((gameData?: any) => {
    trackActivity('slf_game_played', gameData);
  }, [trackActivity]);

  const trackSokratesSession = useCallback((sessionData?: any) => {
    trackActivity('sokrates_session', sessionData);
  }, [trackActivity]);

  const trackBasarTrade = useCallback((tradeData?: any) => {
