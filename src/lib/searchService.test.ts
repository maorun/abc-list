...
describe('SearchService', () => {
  beforeEach(() => {
    ...
    (searchService as any).searchIndex.clear();
    (searchService as any).lastIndexUpdate = 0;
  });
  ...
  describe('Index Building', () => {
    it('should build index from localStorage data', () => {
      ...
      mockLocalStorage.setItem('abcList-TestList:a', JSON.stringify([
        { text: 'Apple', explanation: 'Red fruit' },
        { text: 'Ant', explanation: 'Small insect' }
      ]));
      mockLocalStorage.setItem('Kawas', JSON.stringify([
        { key: 'WORD', text: 'WORD' }
      ]));
      mockLocalStorage.setItem('Kagas', JSON.stringify([
        { key: 'Drawing1', text: 'Drawing1' }
      ]));
  ...
  describe('Search Functionality', () => {
    beforeEach(() => {
      ...
      mockLocalStorage.setItem('abcLists', JSON.stringify(['Animals', 'Science']));
      mockLocalStorage.setItem('abcList-Animals:a', JSON.stringify([
        { text: 'Ant', explanation: 'Small insect', tags: ['insects', 'nature'] },
        { text: 'Apple', explanation: 'Red fruit', tags: ['fruits'] }
      ]));
      mockLocalStorage.setItem('abcList-Science:b', JSON.stringify([
        { text: 'Biology', explanation: 'Study of life', tags: ['science', 'education'] }
      ]));
      
      // Create metadata
      mockLocalStorage.setItem('metadata-abc-list-Animals', JSON.stringify({
        name: 'Animals',
    ...
    it('should find items by tags', () => {
      const results = searchService.search({ query: 'science' });
      expect(results).toHaveLength(1);
      expect(results[0].item.title).toBe('Biology');
    });
    ...
    it('should filter by favorites', () => {
      const results = searchService.search({ isFavorite: true });
      expect(results).toHaveLength(1);
    ...
    it('should filter by category', () => {
      const results = searchService.search({ category: 'Education' });
      expect(results).toHaveLength(1);
      expect(results[0].item.title).toBe('Animals');
    });

    it('should filter by rating', () => {
      const results = searchService.search({ rating: 4 });
      expect(results).toHaveLength(1);
      expect(results[0].item.title).toBe('Animals');
    });
  ...
  describe('Scoring and Ranking', () => {
    beforeEach(() => {
      ...
      mockLocalStorage.setItem('abcList-Test:a', JSON.stringify([
        { text: 'Apple', explanation: 'A red apple fruit' },
        { text: 'Application', explanation: 'Software program' }
      ]));
      
      mockLocalStorage.setItem('metadata-abc-list-Test', JSON.stringify({
        name: 'Test',
        created: '2024-01-01T00:00:00.000Z',
        type: 'abc-list',
        isFavorite: false
  ...
  describe('Date Range Filtering', () => {
    beforeEach(() => {
      ...
      mockLocalStorage.setItem('metadata-abc-list-Recent', JSON.stringify({
        name: 'Recent',
        created: now.toISOString(),
        type: 'abc-list'
      }));
      
      mockLocalStorage.setItem('metadata-abc-list-Old', JSON.stringify({
        name: 'Old',
