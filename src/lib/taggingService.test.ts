...
describe('TaggingService', () => {
  describe('Tag Suggestions', () => {
    ...
    it('should suggest difficulty level tags', () => {
      ...
      const advancedTag = advancedSuggestions.find(s => s.tag === 'Fortgeschritten');
    ...
    it('should generate word-based tags from content', () => {
      ...
      const programmingTag = suggestions.find(s => 
  ...
  describe('Tag Validation', () => {
    it('should validate and clean valid tags', () => {
      ...
      expect(taggingService.validateTag('  computer science  ')).toBe('Computer Science');
      expect(taggingService.validateTag('web-development')).toBe('Web-development');
    });
    ...
    it('should handle non-string input', () => {
      expect(taggingService.validateTag(null as any)).toBeNull();
      expect(taggingService.validateTag(undefined as any)).toBeNull();
      expect(taggingService.validateTag(123 as any)).toBeNull();
    });
  });
  ...
  describe('Tag Categorization', () => {
    it('should categorize science tags correctly', () => {
      const suggestions = taggingService.generateSuggestions(
        ...
        'cell nucleus mitochondria genetics'
      );
    ...
    it('should categorize technology tags correctly', () => {
      ...
      const techTag = suggestions.find(s => 
        s.tag === 'Technik' || s.tag.toLowerCase().includes('computer')
      );
    ...
    it('should categorize language tags correctly', () => {
      ...
      const languageTag = suggestions.find(s => 
        s.tag === 'Sprache' || s.tag.toLowerCase().includes('deutsch')
      );
  ...
  describe('Tag Similarity Detection', () => {
    it('should detect similar tags for cleanup suggestions', () => {
      ...
      const service = taggingService as any;
      
      expect(service.areTagsSimilar('Math', 'Maths')).toBe(true);
      expect(service.areTagsSimilar('Computer', 'Computers')).toBe(true);
      expect(service.areTagsSimilar('Science', 'History')).toBe(false);
    });
  ...
  describe('Content Analysis', () => {
    it('should identify multiple subject areas', () => {
      const suggestions = taggingService.generateSuggestions(
        ...
        'biology chemistry physics mathematics science research'
      );

      const subjectTags = suggestions.filter(s => 
        ['Biologie', 'Chemie', 'Physik', 'Mathematik', 'Wissenschaft'].includes(s.tag)
      );
