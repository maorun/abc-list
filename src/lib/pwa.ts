...
function showUpdatePrompt(registration: ServiceWorkerRegistration): void {
  if (
    confirm("Eine neue Version der App ist verfügbar. Jetzt aktualisieren?")
  ) {
...
export class PWAStorage {
  ...
  private async initDB(): Promise<void> {
    if (!("indexedDB" in window)) {
      console.log(
        "[PWA] IndexedDB not supported, falling back to localStorage",
      );
      return;
    }
  ...
  public async setItem(
    ...
    if (this.db) {
      try {
        ...
        console.error(
          "[PWA] IndexedDB write failed, falling back to localStorage:",
          error,
        );
  ...
  public async getItem(storeName: string, key: string): Promise<any> {
    if (this.db) {
      try {
        ...
        console.error(
          "[PWA] IndexedDB read failed, falling back to localStorage:",
          error,
        );
