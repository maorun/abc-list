...
export function areNotificationsAllowed(
  ...
  if (
    ...
    notificationData.count >= settings.maxNotifications
  ) {
    return false;
  }
 ...
 */
export function shouldCheckForDueReviews(
  settings: NotificationSettings,
): boolean {
  try {
 ...
 */
export function setupPeriodicNotifications(
  getDueReviewsCount: () => number,
): () => void {
  const settings = getNotificationSettings();
