...
class TaggingService {
  ...
  generateSuggestions(
    ...
    const frequentWordTags = this.generateWordBasedTags(
      combinedText,
      existingTags,
    );
    ...
    const difficultyTags = this.generateDifficultyTags(
      combinedText,
      existingTags,
    );
   ...
   */
  private generateDifficultyTags(
    text: string,
    existingTags: string[],
  ): TagSuggestion[] {
    const suggestions: TagSuggestion[] = [];
    ...
    const hasBeginnerTerms = beginnerIndicators.some((term) =>
      text.includes(term),
    );
    const hasAdvancedTerms = advancedIndicators.some((term) =>
      text.includes(term),
    );
  ...
  private generateFormatTags(
    ...
    if (
      title.toLowerCase().includes("vokabeln") &&
      !existingTags.includes("Vokabeln")
    ) {
      suggestions.push({
        tag: "Vokabeln",
        confidence: 0.9,
    ...
    if (
      title.toLowerCase().includes("prüfung") &&
      !existingTags.includes("Prüfung")
    ) {
      suggestions.push({
        tag: "Prüfung",
        confidence: 0.8,
    ...
    if (
      (title + content).toLowerCase().includes("definition") &&
      !existingTags.includes("Definitionen")
    ) {
      suggestions.push({
        tag: "Definitionen",
        confidence: 0.8,
   ...
   */
  getSimilarItemTags(title: string): string[] {
    ...
    for (let i = 0; i < allTags.length; i++) {
      for (let j = i + 1; j < allTags.length; j++) {
        ...
        if (this.areTagsSimilar(tag1, tag2)) {
          const existing = similar.find(
            (s) => s.tags.includes(tag1) || s.tags.includes(tag2),
          );
          if (existing) {
            ...
            similar.push({
              tags: [tag1, tag2],
              suggestion:
                tagStats.get(tag1)! > tagStats.get(tag2)! ? tag1 : tag2,
            });
          }
        }
      }
   ...
   */
  public areTagsSimilar(tag1: string, tag2: string): boolean {
    ...
    return (
      this.levenshteinDistance(t1, t2) <= 2 &&
      Math.abs(t1.length - t2.length) <= 2
    );
  }
