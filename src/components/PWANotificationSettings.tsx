...
import { Bell, BellOff, Settings, TestTube, Check } from 'lucide-react';
...
const handlePermissionRequestAction = (
  ...
  const { basicPermission, pushPermission, subscription } = await requestPWANotificationPermission();
...
const handleTestNotificationAction = (
  setIsTestingNotification: (testing: boolean) => void
) => async () => {
...
export function PWANotificationSettingsDialog() {
  const [settings, setSettings] = useState<PWANotificationSettings>(getPWANotificationSettings());
  ...
  const [support, setSupport] = useState(checkPWANotificationSupport());
  ...
  useEffect(() => {
    ...
    const checkPermissions = () => {
      const basicPermission = 'Notification' in window && Notification.permission === 'granted';
  ...
  const handlePermissionRequest = handlePermissionRequestAction(setPermissions, setSettings);
  const handleTestNotification = handleTestNotificationAction(setIsTestingNotification);
  const handleSettingChange = handleSettingChangeAction(settings, setSettings);

  const canUseNotifications = support.basicNotifications || support.pushNotifications;
  const hasAnyPermission = permissions.basicPermission || permissions.pushPermission;
              ...
              {hasAnyPermission && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <label className="text-sm">Browser-Benachrichtigungen</label>
                    <input
                      type="checkbox"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">
                      Max. Benachrichtigungen pro Tag: {settings.maxNotifications}
                    </label>
                    <input
                      type="range"
                      min="1"
                  ...
                  <Button
                    ...
                    disabled={!settings.enabled && !settings.pushEnabled || isTestingNotification}
                    variant="outline"
                    className="w-full"
                  >
