...
describe("LinkLists", () => {
  ...
  it("displays available lists for selection", () => {
    ...
    localStorageMock.setItem("abcLists", JSON.stringify(["Test List 1", "Test List 2"]));
  ...
  it("allows selecting and deselecting lists", () => {
    ...
    expect(screen.getByText("Listen auswählen (0 ausgewählt)")).toBeInTheDocument();
    
    // Select first list
    fireEvent.click(listButton1);
    expect(screen.getByText("Listen auswählen (1 ausgewählt)")).toBeInTheDocument();
    
    // Select second list
    fireEvent.click(listButton2);
    expect(screen.getByText("Listen auswählen (2 ausgewählt)")).toBeInTheDocument();
  ...
  it("clears all selections when clear button is clicked", () => {
    ...
    // Clear selection
    const clearButton = screen.getByRole("button", {name: "Auswahl zurücksetzen"});
  ...
  it("shows linked lists view when lists are selected", () => {
    ...
    localStorageMock.setItem("abcList-Test List 1:a", JSON.stringify(["apple", "ant"]));
    ...
    expect(screen.getByRole("heading", {level: 3, name: "Test List 1"})).toBeInTheDocument();
  ...
  it("shows bi-associative ideas hint when multiple lists are selected", () => {
    ...
    fireEvent.click(listButton1);
    expect(screen.queryByText("💡 Bi-assoziative Ideen")).not.toBeInTheDocument();
