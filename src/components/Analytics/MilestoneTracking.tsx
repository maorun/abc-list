...
import {CheckCircle, Target, Plus, Trash2, Edit} from "lucide-react";
...
export function MilestoneTracking({data}: MilestoneTrackingProps) {
      ...
      switch (milestone.category) {
        ...
        case "custom":
          if (milestone.id === "complete-alphabet") {
            ...
            current = Math.max(...data.abcLists.map(list => 
              Object.values(list.words).filter(words => words.length > 0).length
            ), 0);
          }
          break;
      }
  ...
  const saveMilestones = (milestonesToSave: Milestone[]) => {
    localStorage.setItem("learningMilestones", JSON.stringify(milestonesToSave));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-green-100 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-green-800">{completedMilestones.length}</div>
          <div className="text-green-600">Erreichte Ziele</div>
        </div>
        <div className="bg-blue-100 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-blue-800">{activeMilestones.length}</div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        <span className="text-gray-600">
                          Fortschritt: {milestone.current} / {milestone.target} {milestone.unit}
                        </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {completedMilestones.map(milestone => (
              <div key={milestone.id} className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <p className="text-sm text-green-700 mb-2">{milestone.description}</p>
                <div className="text-xs text-green-600">
                  Erreicht: {milestone.current} {milestone.unit}
                  {milestone.completedAt && (
                    <span className="ml-2">
                      am {new Date(milestone.completedAt).toLocaleDateString('de-DE')}
                    </span>
                  )}
                </div>
              </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
            <h3 className="text-lg font-semibold mb-4">Neues Lernziel hinzufügen</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewMilestone({...newMilestone, target: parseInt(e.target.value) || 0})}
                    className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    min="1"
                  />
                </div>
      ...
      <div className="bg-gradient-to-r from-yellow-400 to-orange-500 rounded-lg shadow-md p-6 text-white">
        ...
        <div className="space-y-2">
          {completedMilestones.length > 0 && (
            <p>🎉 Fantastisch! Sie haben bereits {completedMilestones.length} Ziele erreicht!</p>
          )}
          {activeMilestones.length > 0 && (
            <p>🎯 Sie arbeiten aktiv an {activeMilestones.length} Zielen - bleiben Sie dran!</p>
          )}
          {milestones.length === 0 && (
            <p>🚀 Setzen Sie sich Ihre ersten Lernziele und starten Sie Ihre Erfolgsreise!</p>
          )}
          <p>💡 Tipp: Kleine, erreichbare Ziele motivieren mehr als große, schwer erreichbare!</p>
        </div>
      </div>
    </div>
  );
