...
export function SmartCollections({ 
  ...
  const CollectionPreview = ({ items, maxItems = 3 }: { items: SearchResult[]; maxItems?: number }) => {
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-xs">
                  {result.item.type === 'abc-list' ? 'Liste' : 
                   result.item.type === 'kawa' ? 'KaWa' :
                   result.item.type === 'kaga' ? 'KaGa' : 'Wort'}
                </Badge>
              </div>
              {result.item.tags.length > 0 && (
                <div className="flex gap-1 mt-1">
                  ...
                  {result.item.tags.length > 2 && (
                    <span className="text-xs text-gray-500">+{result.item.tags.length - 2}</span>
                  )}
                </div>
              )}
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        <h2 className="text-lg font-medium text-gray-900 mb-2">Smart Collections</h2>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="font-medium text-blue-800 mb-3">📊 Sammlungsstatistiken</h3>
        ...
        <div className="mt-4 pt-3 border-t border-blue-200">
          <div className="text-xs text-blue-600 space-y-1">
            <p>💡 <strong>Tipp:</strong> Smart Collections werden automatisch aktualisiert</p>
            <p>🏷️ <strong>Organisation:</strong> Verwende Tags für bessere Kategorisierung</p>
            <p>⭐ <strong>Favoriten:</strong> Markiere wichtige Inhalte als Favoriten</p>
          </div>
        </div>
      </div>
    </div>
